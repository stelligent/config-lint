...
func RunTestTerraformBuiltInRules(t *testing.T, terraformVersion string) {
	...
	testCases := []BuiltInTestCase{
		...
		{"tf", "aws/kinesis_firehose_delivery_stream/encryption.tf", "KINESIS_FIREHOSE_DELIVERY_STREAM_ENCRYPTION", 0, 4},
		{"tf", "aws/kinesis_stream/encryption.tf", "KINESIS_STREAM_ENCRYPTION", 0, 2},
		{"tf", "aws/kinesis_stream/kms_key.tf", "KINESIS_STREAM_KMS", 1, 0},
		{"tf", "aws/redshift_cluster/encrypted.tf", "REDSHIFT_CLUSTER_ENCRYPTION", 0, 2},
		{"tf", "aws/redshift_cluster/kms_key_id.tf", "REDSHIFT_CLUSTER_KMS_KEY_ID", 1, 0},
		{"tf", "aws/redshift_cluster/enhanced_vpc_routing.tf", "REDSHIFT_CLUSTER_ENHANCED_VPC_ROUTING", 2, 0},
		{"tf", "aws/redshift_cluster/logging.tf", "REDSHIFT_CLUSTER_AUDIT_LOGGING", 2, 0},
		{"tf", "aws/redshift_cluster/publicly_accessible.tf", "REDSHIFT_CLUSTER_PUBLICLY_ACCESSIBLE", 0, 2},
		{"tf", "aws/redshift_parameter_group/require_ssl.tf", "REDSHIFT_CLUSTER_PARAMETER_GROUP_REQUIRE_SSL", 2, 0},
