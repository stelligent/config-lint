...
func RunTestTerraformBuiltInRules(t *testing.T, terraformVersion string) {
  ...
  // Test cases can be applied to tf11, tf12, or both
	testCases := []BuiltInTestCase{
		...
		{"both", "aws/s3.tf", "S3_BUCKET_ACL", 0, 0},
		{"both", "aws/s3.tf", "S3_NOT_ACTION", 0, 0},
		{"both", "aws/s3.tf", "S3_NOT_PRINCIPAL", 0, 0},
		{"tf", "aws/s3_bucket_policy/policy_version.tf", "POLICY_VERSION", 0, 1},
		{"tf", "aws/s3.tf", "S3_BUCKET_POLICY_WILDCARD_PRINCIPAL", 1, 0},
		{"tf", "aws/s3.tf", "S3_BUCKET_POLICY_WILDCARD_ACTION", 1, 0},
		{"both", "aws/s3.tf", "S3_BUCKET_ENCRYPTION", 0, 1},
		{"both", "aws/s3.tf", "S3_BUCKET_OBJECT_ENCRYPTION", 0, 1},
		{"tf", "aws/sns_topic_policy/policy_version.tf", "POLICY_VERSION", 0, 1},
		{"tf", "aws/sns.tf", "SNS_TOPIC_POLICY_WILDCARD_PRINCIPAL", 1, 0},
		{"both", "aws/sns.tf", "SNS_TOPIC_POLICY_NOT_ACTION", 0, 0},
		{"both", "aws/sns.tf", "SNS_TOPIC_POLICY_NOT_PRINCIPAL", 0, 0},
		{"tf", "aws/sqs_queue_policy/policy_version.tf", "POLICY_VERSION", 0, 1},
		{"tf", "aws/sqs.tf", "SQS_QUEUE_POLICY_WILDCARD_PRINCIPAL", 1, 0},
		{"both", "aws/sqs.tf", "SQS_QUEUE_POLICY_WILDCARD_ACTION", 0, 0},
		{"both", "aws/sqs.tf", "SQS_QUEUE_POLICY_NOT_ACTION", 0, 0},
		{"both", "aws/sqs.tf", "SQS_QUEUE_POLICY_NOT_PRINCIPAL", 0, 0},
		{"both", "aws/sqs.tf", "SQS_QUEUE_ENCRYPTION", 0, 1},
		{"both", "aws/lambda.tf", "LAMBDA_PERMISSION_INVOKE_ACTION", 0, 0},
		{"both", "aws/lambda.tf", "LAMBDA_PERMISSION_WILDCARD_PRINCIPAL", 0, 0},
		{"both", "aws/lambda.tf", "LAMBDA_FUNCTION_ENCRYPTION", 1, 0},
		{"both", "aws/lambda.tf", "LAMBDA_ENVIRONMENT_SECRETS", 0, 1},
		{"both", "aws/waf.tf", "WAF_WEB_ACL", 0, 0},
		{"both", "aws/alb.tf", "ALB_LISTENER_HTTPS", 0, 3},
		{"both", "aws/alb.tf", "ALB_ACCESS_LOGS", 0, 0},
		{"both", "aws/ami.tf", "AMI_VOLUMES_ENCRYPTED", 0, 1},
		{"both", "aws/ami.tf", "AMI_COPY_SNAPSHOTS_ENCRYPTED", 0, 1},
		{"both", "aws/ec2.tf", "EBS_BLOCK_DEVICE_ENCRYPTED", 0, 0},
		{"both", "aws/ec2.tf", "EBS_VOLUME_ENCRYPTION", 0, 2},
		{"tf", "aws/cloudtrail.tf", "CLOUDTRAIL_ENCRYPTION", 0, 1},
		...
		{"both", "aws/codepipeline.tf", "CODEPIPELINE_ENCRYPTION", 0, 0},
		{"both", "aws/db.tf", "DB_INSTANCE_ENCRYPTION", 0, 1},
		{"both", "aws/db.tf", "RDS_CLUSTER_ENCYPTION", 0, 2},
